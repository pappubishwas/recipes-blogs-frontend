  ...
  return (
    <div className="h-16 flex justify-between items-center px-6 lg:px-12">
      ...
      <div className={`transition-all w-full h-full fixed bg-primary z-50 top-0 ${hideLeft} flex justify-center items-center`}>
            ...
            <ul className="flex flex-col gap-5">
                {
                    menuItems?.map((menu,index)=>(
                        <li key={index}>
                            <Link to={menu} className="font-medium capitalize secondary">{menu}</Link>
                        </li>
                    ))
                }
            </ul>
        ...
        <li>
            <button className="secondary px-4 py-2 rounded border"> Log in</button>
        </li>
        <li>
            <button className="secondary px-4 py-2 rounded border"> Sign up</button>
        </li>
      </ul>
        </div>
      </div>
